    ...
    <main className="min-h-screen bg-[#080f0d] text-white">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="marketing-split relative mx-auto min-h-[calc(100vh-4rem)] max-w-7xl px-5 py-16 sm:px-6 lg:px-8 lg:py-20">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 shadow-[0_24px_70px_rgba(0,0,0,0.24)] sm:p-5">
            <img src="/demo/property-cover.svg" alt="" className="aspect-[5/3] w-full rounded-xl border border-white/10 object-cover" />
            <div className="marketing-metric-grid mt-4">
              {[
                ...
                <div key={label} className="rounded-xl border border-white/10 bg-[#0d1714] p-4">
      ...
      {offerings.map((item, index) => {
        ...
        return (
          <section key={item.id} id={item.id} className="scroll-mt-16 border-b border-white/10">
            <div className="marketing-split mx-auto min-h-screen max-w-7xl px-5 py-20 sm:px-6 lg:px-8">
              <div>
                ...
                <h2 className="marketing-section-title mt-4 max-w-2xl font-[var(--font-display)] text-white">{item.title}</h2>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 shadow-[0_20px_56px_rgba(0,0,0,0.16)] sm:p-6">
                <div className="content-split-tight">
                  <div className="rounded-2xl border border-white/10 bg-[#0d1714] p-6">
                    <p className="text-xs uppercase tracking-[0.22em] text-white/[0.34]">Signal</p>
                    <p className="mt-8 font-[var(--font-display)] text-[clamp(2.75rem,5vw,3.5rem)] text-white">{item.stat}</p>
                    <p className="mt-3 text-sm leading-6 text-white/[0.56]">{item.statLabel}</p>
                  </div>
                  <div className="space-y-3">
                    {item.rows.map((row) => (
                      <div key={row} className="flex items-center justify-between rounded-xl border border-white/10 bg-[#0d1714] px-4 py-3">
      ...
      <section className="marketing-feature-grid mx-auto max-w-7xl px-5 py-16 sm:px-6 lg:px-8">
        {[
