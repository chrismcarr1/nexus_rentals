    ...
    return (
      <div className="space-y-5">
        ...
        <section className="grid gap-px border border-[var(--line)] bg-[var(--line)] md:grid-cols-2 xl:grid-cols-3">
          <AdminHealthCard label="Database" healthy={data.system.databaseConnected && data.system.databaseConfigured} detail={data.system.databaseTarget === "local-json" ? "Local development data is isolated in the ignored data/app-db.json file." : "The admin snapshot was read successfully from the hosted store. Hostname only; credentials are never displayed."} value={data.system.databaseConfigured ? data.system.databaseTarget : "DATABASE_URL missing"} />
