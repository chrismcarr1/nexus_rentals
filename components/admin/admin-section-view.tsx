    ...
    return (
      <div className="space-y-5">
        ...
        <section className="grid gap-px border border-[var(--line)] bg-[var(--line)] md:grid-cols-2 xl:grid-cols-3">
          <AdminHealthCard label="Database" healthy={data.system.databaseConnected && data.system.databaseConfigured} detail="The admin snapshot was read successfully from the hosted store." value={data.system.databaseConfigured ? "DATABASE_URL present" : "DATABASE_URL missing"} />
          ...
          <AdminHealthCard label="Stripe API" healthy={data.system.stripeApiConfigured} detail="Presence check only. Secret values are never displayed." value={data.system.stripeApiConfigured ? "STRIPE_SECRET_KEY present" : "STRIPE_SECRET_KEY missing"} />
          ...
          <AdminHealthCard label="Runtime environment" healthy={true} detail="Current application runtime mode." value={data.system.environment} />
